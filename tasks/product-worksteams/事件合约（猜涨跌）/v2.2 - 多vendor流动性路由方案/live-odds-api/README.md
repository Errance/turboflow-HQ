...
```text
predict-market-live-odds.mjs
```

Run once and exit after fetching the contract list:

```sh
node predict-market-live-odds.mjs --once
```

Run live for 15 seconds:

```sh
node predict-market-live-odds.mjs --duration-ms 15000
```

Filter to one pair id:

```sh
node predict-market-live-odds.mjs --pair-id 6 --duration-ms 15000
```

Override endpoints:

```sh
node predict-market-live-odds.mjs \
